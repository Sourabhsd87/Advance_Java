...
public class LoginServlet extends HttpServlet {
	...
	public void init() throws ServletException {
		try {
			...
			//ServletException(string msg , throwable rootcause)
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			if (user == null) // login failed
				...
				//create a cookie to hold the user data
				//javax.servlet.http.cookie(string nm,string value)
		...
		//		pw.flush();//sending the resp (commiting the resp)
				...
				if (user.isVotingStatus()) {// alrdy voted
					...
					//WC clears(emties or discards) the resp buffer 
					//sends temp redirect resp : SC 302 | location=candidate_list,Set-Cookie(user_dtls,user.toString()) | body : EMPTY
					//web browser checks privacy settings-->cookies blocked-->session tracking blocked else
					//checks cookie age--> -1:stores the cookie in cache
					//web browser : sends NEXT request : http://host:port/day2.1/candidate_list, method =GET
					//request header: cookie:user_dtls,user.toString()
